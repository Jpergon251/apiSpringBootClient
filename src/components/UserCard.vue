<template>
  <section class="user" v-if="user.username !== myUser.username">
    <section
      v-if="!editingUserId || editingUserId !== user.id"
      class="user-data"
    >
    ...
    <form
      @submit.prevent="saveEditedUser(user)"
      v-if="editingUserId === user.id"
      class="user-data editing"
    >
      <p class="error-message">{{ errormessage }}</p>
      <input
        v-model="this.editedUsername"
        class="username"
        :placeholder="user.username"
      />

      <input
        v-model="this.editedEmail"
        class="email"
        :placeholder="user.email"
      />

      <select class="select-role role" v-model="this.editedRole" required>
        <option :value="user.role" disabled="disabled" selected="true">
          {{ user.role }}
        </option>
      ...
      <section class="edit-buttons">
        <button class="save-but" @click="saveEditedUser(user)">
          Guardar Cambios
        </button>
        <button class="cancel-but" @click="cancelEditUser(user)">
          Cancelar
        </button>
      </section>
    </form>
    ...
    <section class="users-buttons">
      <button class="edit-but" @click="toggleEditUser(user)">
      ...
      <button class="delete-but" @click="deleteUser(user)">
  ...
  methods: {
    ...
    async blockUser(user) {
      ...
      try {
        ...
        const blockUser = await axios.put(
    ...
    async unblockUser(user) {
      try {
        ...
        const unBlockUser = await axios.put(
          `http://localhost:8080/users/${user.id}`,
          user,
          this.configToken
        );
